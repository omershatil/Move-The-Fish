 ...
 *
 */
@Service("autoSchoolFishMover")
public class AutoSchoolFishMoverImp implements AutoFishMover, Runnable {
	...
	private int schoolWidth = 240;
	@Value("${fish.tank.h}")
	private String tankHeightString;
	private int tankHeight = 900;
	private int schoolHeight = 200;
	@Value("${school.fish.move.max.ms.delay}")
    ...
    public void init() {
		...
		for (AnimatedEntity fish: fishManager.getAllFish()) {
			if (fish.getType().equalsIgnoreCase("SCHOOL_FISH") == true) {
				setPosition(fish);
	...
	public void updateFishLocation(AnimatedEntity fish) {
		// do nothing! let fish go back to its initial position.
	}
	
	private void setPosition(AnimatedEntity fish) {
		// fish is ready to be moved. set its new position
		int x = Math.abs(new Random().nextInt() % this.schoolWidth);
		if ((x + fish.getW()) > this.schoolWidth) x = this.schoolWidth - fish.getW();
		int y = Math.abs(new Random().nextInt() % this.schoolHeight);
		if ((y + fish.getH()) > this.schoolHeight) y = this.schoolHeight - fish.getH();
